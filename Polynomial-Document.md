...
封装了一个多项式模板类，已上传至 [GitHub 仓库的 Polynomial 文件](https://github.com/ForAurie/QMath/blob/main/Polynomial)，`include` 该文件或粘贴至代码开头即可使用，最低支持 C++17。
...
4. 强大的功能
    ...
    它顺便实现了多项式 `exp`、`ln`、`sqrt` 等功能。还使用 FWT、FMT 实现了与、或、非、异或卷积，不久的将来会添加集合幂级数相关功能。
...
// 如果要构造该类型可以这样
Modular<int, 998244353, unsigned long long> a(1); // 初始化为 1
...
typedef QMath::Polynomial<Mint, Mint, T2NTT, NTT2T, NTT> Poly;
```
...
int main() {
    ...
    int n, m;
    cin >> n >> m;
    QMath::Polynomial<Mint, Mint, T2TNTT, TNTT2T, NTT> a(n + 1), b(n + 1, Mint(0)); // 构造函数和 vector 是一样的，这两种都可以。
...
* 实测加上这一优化后模板题的表现：FFT 从 580 ms 左右优化至 520 ms 左右，但 NTT 从 480 ms 左右劣化到 510 ms 左右。
    ...
    问题原因分析：迭代式 Cache-unfriendly 的原因是因为每次换层都要重新开始遍历多项式数组，需要把多项式末尾的从缓存移出，开头的移进去（能否分奇偶层使用不同方向遍历呢？改天试试）。递归式则由于优秀的递归顺序避免了这个问题，NTT 使用 `int` 存储，`int` 占空间小，在题目数据范围下几乎整个多项式都能被扔到 L2 缓存里面，因此优化 Cache 对其效果不明显，反而会，但 FFT 使用了 `std::complex<double>` 存储，空间占用是 `int` 的 $4$ 倍，L2 缓存放不下，因此 Cache 对其优化效果明显。

    解决方案：可以短序列用迭代、长序列用递归。但只能这样吗？止步于此了吗？虽然 NTT / FFT 的变换实现分为迭代式和递归式，但它们做的事情本质相同，只是访问数序不同，为什么不能在一个序列上同时使用迭代和递归呢？因此我的混合使用了迭代式和递归式：先使用递归实现，进入递归时判断目前递归区间的长度，若长度很大，则继续递归，如果长度小于一个阈值（本代码取 $4096$），短到即便是使用迭代式实现也能完全卡进 L2 缓存，则停止递归剩下的全部迭代实现，此时递归结构可能如下：
    ...
    `-` 部分区间长度大，采用递归。`+` 部分区间很小，直接迭代完成。可以看到递归树顶部是递归算的，底部是迭代算的，虽然遍历方式不一样，但做的事情都一样，因此不影响正确性。

* 支持的多项式长度范围有限，且因为编译期间展开长多项式的递归函数会导致编译文件体积暴增，因此不能支持特别长的多项式，虽在 $998244353$ 下可以接受，但是违背了泛型思想。

    解决方案：如果发现多项式过长，则在递归结构的最高最长的几层放弃优化，直接使用迭代计算，待问题规模缩小至递归可以时开始递归，本代码可递归长度的上限是 $2^{24}$，洛谷模板题其实没有超过这个上限。